...
def p1(arr_init):
    res = 0
    phase = None
    for phase_values in it.permutations(range(5)):
        amps = [Amp(ph, arr_init.copy(), debug=True) for ph in phase_values]
        val = 0
...
def p2(phase_value, arr_init):
    ...
    values = [0] * num_values
    amps = [Amp(f'{c}{ph}', arr_init.copy(), debug=True) for c, ph in zip('ABCDE', phase_value)]
    ...
    while True:
        for i in range(5):
            ...
            if out is not None:
                v = out
            ic(f"{_i}:  Amp {amps[i].id} values={values} idx: {amps[i].i} arr: {amps[i].arr} in:{list(amps[i].input_value.queue)}")
