...
% word in chunk iter variable
i=1;
% chunk in string iter variable
k=1;
% tuple with size of string as: (rows, cols)
nn = size(str);
...
%while i<2
    ...
    vf(k) = calcf(frag,len);
    i = i+wid; k=k+1;
    %disp(strcat('Total Chars: ', int2str(i)))
end
disp(strcat('Total Fragments: ', int2str(k)))
