...
% Universal mean and sigma ?
um = mean(S, 'all');
ustd = sqrt(var(double(S), 0, 'all'))  % std does not operate over all values
...
figure(2);
for i=1:M
 str=strcat('pics/norms/', int2str(i), '.jpg');
...
for i=1:size(u,2)
 ...
 img=img';
 img=histeq(img,255);
 str = strcat('pics/eigenimages/', int2str(i), '.jpg');
 eval('imwrite(img,str)');
...
img=imread('pics\instructors\broderick.jpg')
img=imread('pics\instructors\1.jpg')
if ndims(img) == 3
...
for i=1:size(u,2);
% This is a guess
 Difference = double(ReshapedImage(:,i)) - double(meanImg(:,i));
 t = u(:,i)';
